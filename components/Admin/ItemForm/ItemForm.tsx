...
const ItemForm: React.FunctionComponent<IItemForm> = ({
  show,
  sending,
  item,
  close,
  ...
  const addHandler = () => {
    let selectedLanguages = Object.keys(form.language.options).filter(
      (option) => form.language.options[option].selected
    );
