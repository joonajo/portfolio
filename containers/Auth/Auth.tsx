...
const Auth: React.FunctionComponent<IAuthComponent> = ({
  setSending,
  signIn,
}): JSX.Element => {
    ...
    fetch(baseURL + apiKey, { method: "post", body: payload })
      ...
      .then((data) => {
        if (data.error) {
          ...
          const expireTime: string = String(
...
const AuthForm: React.FunctionComponent<IAuthForm> = ({
  ...
  const [formState, setFormState] =
    React.useState<IFormState>(initialAuthFormState);
  ...
  return (
    <div className={css.AuthFormContainer}>
      ...
      {error?.error && (
        <p className={css.ErrorText}>{error.errorInfo?.message}</p>
      )}
